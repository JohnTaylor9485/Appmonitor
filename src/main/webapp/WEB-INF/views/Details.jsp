...
<%@ page import="com.skylon.Appmonitor.entity.CompleteInformation" %>
<%@ page import="java.text.SimpleDateFormat" %>
<%@ page pageEncoding="UTF-8" %>

<input type="button" value="返回" onclick=window.location.href="/appmonitor/status";>
...
<table id="datatable" border="1" cellspacing="0" cellpadding="0" width="100%" style="align:center;">
    ...
    <%
        //循环显示数据
        List<CompleteInformation> datalist = (List) request.getAttribute("resultList"); // 取request里面的对象队列
        if (datalist.size() != 0) {
    ...
    <tr>
        ...
        <td><%=datalist.get(i).getProjName() %>
        </td>
        <td><%=datalist.get(i).getProgramCode() %>
        </td>
        <td><%=datalist.get(i).getAppName() %>
        </td>
        <td><%=datalist.get(i).getParameterCode() %>
        </td>
        <td><%=datalist.get(i).getParamName() %>
        </td>
